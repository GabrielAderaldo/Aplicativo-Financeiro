...
import kotlinx.android.synthetic.main.activity_despeza.*
...
class DespesaActivity : AppCompatActivity() {
    ...
    fun salvarDados(){
        if(inpultReceitaNome.text.toString().isNotEmpty() && inpultReceita.text.toString().isNotEmpty()){

            var transacoes = Transacoes(inpultReceitaNome.text.toString(),-inpultReceita.text.toString().toDouble())
            var dataBase = Firebase.database.reference
            dataBase.child("usuarios").child(auth.currentUser!!.uid).child("despesas").push().setValue(transacoes)
            Toast.makeText(baseContext,"O valor: "+inpultReceita.text.toString().toDouble()+" Foi salvo com sucesso!", Toast.LENGTH_SHORT).show()

        }else{
            Toast.makeText(baseContext,"Verifique se você digitou corretamente todos os campos...",
